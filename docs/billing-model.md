...
Additional (metered / `overage`) usage only occurs if the enterprise/organization **"AI credit paid usage"** policy is enabled — a **real GitHub governance control**, managed as a Copilot policy in enterprise/organization settings (the enterprise **"AI controls"** tab). [B1][B4][B17] When it is disabled, usage is **blocked** as soon as the shared pool is exhausted — regardless of any budget, and regardless of an included-usage cap set to `overage`. [B1][B4] The simulator assumes paid usage is **allowed** and instead models hard stops via the budget "stop usage" flags (`formulas.md` §6c) and the included-cap block mode. Because it maps to a real governance control (not an invented knob), modeling it **satisfies the §2 design invariant** and is a valid extension point: a global **"AI credit paid usage"** toggle that, when off, forces all post-pool demand to block.
