...
- **Paid-usage policy gates all metered.** Before any budget cap, `ApplyMetered` checks the global enterprise/org **"AI credit paid usage"** policy (`allowPaidUsage`): if off, it serves **no** metered credits at all, so every post-pool request blocks and the max bill collapses to license fees. This is a global gate, not a cost-center control (§5.6 of billing-model). [B1][B4]
...
- **Additivity.** Because budgets cap only metered charges, the worst-case bill is $M=F+\beta_E$ (or just $F$ if paid usage is off), not $\beta_E$ — the max-bill rule. [B5]
