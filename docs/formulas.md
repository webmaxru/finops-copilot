...
The enterprise limit is an absolute USD budget on the slider range $[0,\ \beta_E^{\max}(L)]$. The **default** is derived from a **reference run of the shipped default inputs at $v=0$** (no usage variation) with **standard allowances** (promo off — the default anchors the durable post-promo regime), budgets non-binding:
...
At the shipped defaults ($v=0$): **72 normal** users consume \$50 each (target \$50 = universal ULB) and **8 power** users consume \$190 each (their individual budget), so $U_{\text{ref}}=72(\$50)+8(\$190)=\$5{,}120$. The \$2,500 **standard** pool ($P\cdot c$) is exceeded, so $\text{metered}_{\text{ref}}=\$2{,}620$ ⇒ **default $=\$2{,}620$**.
