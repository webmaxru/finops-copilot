...
Every billing rule in these docs is a **[Fact]** backed by an official GitHub source below. Tags `[B1]`–`[B18]` are used throughout [`billing-model.md`](./billing-model.md), [`formulas.md`](./formulas.md), and [`simulation-engine.md`](./simulation-engine.md). Verified 2026-07 against the June 1 2026 usage-based billing model. **All `docs.github.com` links use the `enterprise-cloud@latest` version** (Copilot Business/Enterprise on GitHub Enterprise Cloud), not the free/pro/team version; product-release notes are on `github.blog/changelog`.
...
| **B4** | Budget scopes/hierarchy; user-level budgets cap total & always hard-stop; per-request evaluation order; lowest-headroom-wins | https://docs.github.com/en/enterprise-cloud@latest/copilot/concepts/billing/budgets-for-usage-based-billing |
...
| **B6** | "Stop usage when budget limit is reached" is OFF by default; alert thresholds 75/90/100% | https://docs.github.com/en/enterprise-cloud@latest/billing/concepts/budgets-and-alerts |
...
| **B13** | Cost centers support an AI credit pool cap (single `ai_credit_pool_enabled` boolean, auto-sized to 3,000/7,000 AICR per license; behavior at the cap = the enterprise overages policy, **not** a per-CC block/overage field; API-only, UI early July) | https://github.blog/changelog/2026-07-02-cost-centers-now-support-included-usage-caps/ |
...
| **B17** | Copilot policies are governance controls in the enterprise **"AI controls"** tab / organization settings (feature, model, agent & usage policies) | https://docs.github.com/en/enterprise-cloud@latest/copilot/concepts/policies |
| **B18** | Enterprise-scoped budgets can **exclude cost-center usage** (single enterprise-budget setting; CC usage excluded collectively; default includes; REST + audit log) | https://github.blog/changelog/2026-01-19-enterprise-scoped-budgets-that-exclude-cost-center-usage-in-public-preview/ |
