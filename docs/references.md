...
Every billing rule in these docs is a **[Fact]** backed by an official GitHub source below. Tags `[B1]`–`[B20]` are used throughout [`billing-model.md`](./billing-model.md), [`formulas.md`](./formulas.md), and [`simulation-engine.md`](./simulation-engine.md). Verified 2026-07-06 (firsthand fetch of B1–B18; **B19** added 2026-07-08; **B20** added 2026-07-20) against the June 1 2026 usage-based billing model. **All `docs.github.com` links use the `enterprise-cloud@latest` version** (Copilot Business/Enterprise on GitHub Enterprise Cloud), not the free/pro/team version; product-release notes are on `github.blog/changelog`.
...
| **B8** | REST cost centers; `ai_credit_pool_enabled` boolean + auto-sized `ai_credit_pool_state` (`target_amount`/`current_amount`) — the **only** pool parameters in the POST body. The per-cost-center **block/overage at the cap** choice (**B20**) is **not** in this body; it is set via the billing-UI budget/spend-control flow (`billing-model.md` §6) | https://docs.github.com/en/enterprise-cloud@latest/rest/billing/cost-centers |
...
| **B10** | Cost center concept; **included-usage controls** cap a CC to its own licenses' credits; **at the cap you choose, per cost center, block vs. paid overage** (**B20**/**B4**), the overage branch gated by the enterprise overages policy; a resource belongs to only one cost center at a time (⇒ Σ members ≤ licenses — the explicit rule is on the *use-cost-centers* how-to, Additional context below) | https://docs.github.com/en/enterprise-cloud@latest/billing/concepts/cost-centers |
...
| **B12** | Per-user AI credit budgets for cost centers (introduced via REST API; now also in the billing UI — see **B19**) | https://github.blog/changelog/2026-06-30-per-user-ai-credit-budgets-available-for-cost-centers/ |
| **B13** | Cost centers support an AI credit pool cap: a single `ai_credit_pool_enabled` boolean, auto-sized to the CC's own licenses' included credits (Business 3,000 / Enterprise 7,000 during promo; 1,900/3,900 std). Introduced **API-first** (2026-07-02); its "block … or let it continue as additional spend if your enterprise allows overages" wording is the per-cost-center at-cap choice later surfaced in the billing UI by **B20** (2026-07-20). See `billing-model.md` §6. | https://github.blog/changelog/2026-07-02-cost-centers-now-support-included-usage-caps/ |
...
| **B18** | Enterprise-scoped budgets can **exclude cost-center usage** — a single **"Exclude cost center usage from this budget"** checkbox on the enterprise budget (Preview) that excludes **all** cost-center usage from that budget collectively; each cost center is then bounded only by its own CC budget. Default **includes** CC usage; available via REST and audit-logged | https://github.blog/changelog/2026-01-19-enterprise-scoped-budgets-that-exclude-cost-center-usage-in-public-preview/ |
| **B19** | Per-user budgets for cost centers are now available in the **billing UI** (GitHub Enterprise Cloud) — the same per-user AI-credit budget control previously REST-API-only (**B12**); add enterprise teams or individual users to a cost center, set **one** per-user budget that applies to everyone in it, and coverage stays in sync as membership changes | https://github.blog/changelog/2026-07-07-per-user-budgets-for-cost-centers-in-the-billing-ui/ |
| **B20** | **AI credit pools for cost centers in the billing UI**: you manage a cost center's AI credit pool when creating/editing it in the billing UI (GitHub auto-calculates the pool from assigned licenses, no number to enter), and **choose, per cost center, what happens at the limit — block further included usage, or let it continue as additional spend if your enterprise allows overages** | https://github.blog/changelog/2026-07-20-ai-credit-pools-for-cost-centers-in-the-billing-ui/ |
...
The simulator splits blocked users by the **binding hard stop** — `userLimit`, `includedCap`, `costCenterBudget`, or `enterpriseBudget` (see [`formulas.md`](./formulas.md) §6d, §7.1). This introduces **no new billing rule**: the stops are already-cited facts — user-level budgets always hard-stop [B4][B16], a cost-center AI-credit-pool cap set to **"block" at the cap** hard-stops there [B20][B4], and cost-center/enterprise metered-budget "stop usage" flags hard-stop the metered leg [B4][B6]. (When the cap is set to **overage** instead — the simulator's default, assuming the enterprise allows overages — the excess spills to metered and adds no block; see `billing-model.md` §6.) Which stop is reported as *the* reason (user limit first, then an included-cap block, then whichever budget `ApplyMetered` found binding) is a **[Derived]** attribution / modeling choice, surfaced in chart tooltips and the global "Blocked users" KPI. Validated in `src/model/__tests__/engine.test.ts` ("blocked-user reason breakdown" and "cost-center AI credit pool").
